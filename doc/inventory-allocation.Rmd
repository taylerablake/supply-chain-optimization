...
```{r, ref.label="discountAUR", fig.cap="A typical curve for describing the relationship between the price of excess units and the overstock quantity - the amount by which the buy exceeds demand.",fig.align="center",out.width = "190px",out.height="190px"}
library(ggplot2)
library(magrittr)
...
data.frame(buy=b,AUR=AUR) %>%
     ggplot(data.frame(buy=b,AUR=AUR), aes(x=buy,y=AUR)) +
     geom_line() +
     theme_minimal() +
