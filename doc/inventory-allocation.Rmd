...
\begin{itemize}
\item Optimizing the distribution of retail inventory to fulfillment centers requires forecasting future product demand, and there is uncertainty associated with any forecast.
\item Forecast uncertainty comes from multiple sources, but current inventory allocation tools typically only account for a portion of this uncertainty.
\item We applied Bayesian methodology to optimize allocation while accounting for all sources of uncertainty in a forecast.
\end{itemize}
...
```{r,echo=FALSE, error=FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.width = 8,fig.height=3,fig.align="center", fig.cap = "Several hypothetial 'true' relationships between temperature and coffee sales."}
library(lubridate)
library(ggplot2)
library(ggthemes)
library(magrittr)
