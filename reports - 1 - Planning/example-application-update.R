...
data.frame( expand.grid(t=1:n_time_periods,region=factor(1:n_regions)),
...
ggplot(curve_frame) +
  ...
  theme_minimal()
ggsave("reports - 1 - Planning/white-paper-figures/demand-curve-95pct.png",
       height = 4, width = 6)
...
#######################################################################################
## grab two realizations from the posterior samples matrix and the corresponding sales
## for each region

## under each sample from the posterior:
##  map the total sales to households in each region;
##  determine the optimal allocation of inventory

## take the "true" value of theta to be the posterior means
##  for this state of nature, determine the actual sales


##
## visualize the discrepancy between the inventory allocated
## for each of the posterior samples and the true demand
#######################################################################################
