...
ggmap(us_gg_map) +
  ...
  #              alpha = .5) +
  geom_point(data = data.frame(household_locations,
                               assigned_fc = as.factor(fc_to_hh_distance))[sample(nrow(household_locations), 10000), ],
             aes(x = x, y = y, color = assigned_fc),
             size = .5) +
  geom_point(data = fulfill_centers_locations,
             aes(x = x, y = y), color = '#FFFFFF',
             size = 5) +
...
ggmap(us_gg_map) +
  ...
  geom_text(data = fulfill_centers_locations, 
            aes(x = x, y = y, label = fc), color = "#000000",
            size = 2)
ggsave("reports - 1 - Planning/microstrategy-symposium-images/map-only.png",
       height = 5.5, width = 9.17)
