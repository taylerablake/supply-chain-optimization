...
region[household_locations$state=="Louisiana"] <- "West Region"
region[household_locations$state=="Texas"] <- "West Region"
region[household_locations$state %in% c("Missouri","Iowa","Arkansas","Minnesota")] <- "East Region"
...
ggplot() +
  ...
  guides(colour="none") +
  scale_color_tableau() 
ggsave("doc/img/two-region-household-map.png",
...
ggplot() +
  ...
  geom_polygon(data = alpha_polys,
               aes(x = c1, y = c2, group=fc, fill="white"),
               colour="white",
               alpha=0.001)
