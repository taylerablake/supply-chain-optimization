...
storage <- data.frame(total_value = rep(NA, 2),
                            amt_sold = rep(NA, 2),
                            total_buy = rep(NA, 2),
                            total_demand = rep(NA, 2),
                            total_delivery_cost = rep(NA, 2),
                            #total_transfer_cost = rep(NA, 2),
                            full_price = rep(NA, 2),
                            marked_down = rep(NA, 2))
...
storage$total_value[1] <- total_value
storage$amt_sold[1] <- amt_sold
storage$total_buy[1] <- total_buy
storage$total_demand[1] <- total_demand
storage$total_delivery_cost[1] <- total_delivery_cost
#storage$total_transfer_cost[this_generation] <- total_transfer_cost
storage$full_price[1] <- amt_sold * AUR
storage$marked_down[1] <- (total_buy - amt_sold) * AUR * exp(-delta * (total_buy - amt_sold)) * (total_buy >= total_demand)
...
storage$total_value[2] <- total_value
storage$amt_sold[2] <- amt_sold
storage$total_buy[2] <- total_buy
storage$total_demand[2] <- total_demand
storage$total_delivery_cost[2] <- total_delivery_cost
#storage$total_transfer_cost[this_generation] <- total_transfer_cost
storage$full_price[2] <- amt_sold * AUR
storage$marked_down[2] <- (total_buy - amt_sold) * AUR * exp(-delta * (total_buy - amt_sold)) * (total_buy >= total_demand)


data.frame(storage,data.frame(rbind(avgs[which.max(avgs$ev),1:5],
                 naive_avgs[which.max(naive_avgs$ev),1:5])))
